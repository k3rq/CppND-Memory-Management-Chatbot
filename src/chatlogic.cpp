...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            _edges.push_back(std::make_unique<GraphEdge>(edge->GetID()));

                            // find all keywords for current node
                            AddAllTokensToElement("KEYWORD", tokens, *edge);

                            // store reference in child node and parent node
                            (*childNode)->AddEdgeToParentNode(std::make_unique<GraphEdge>(edge->GetID()));
                            (*parentNode)->AddEdgeToChildNode(std::move(std::make_unique<GraphEdge>(edge->GetID())));
                        }
    ...
    //Issue appears to occur at ChatBotPanelDialogItem::ChatBotPanelDialogItem, line 198
